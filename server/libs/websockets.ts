  ...
  io.on('connection', (socket) => {
    ...
    socket.on('fetchKeyData', async (key: string, callback) => {
      ...
      try {
        ...
        const exists = await redis.sendCommand('JSON.GET', [`chat:${key}`]);
        ...
        const { activeUsers, maxUsers, users } = reply as unknown as Key;
    ...
    socket.on('joinChat', async (key: string, name: string, avatar: string, callback) => {
      ...
      try {
        ...
        if (await redis.sendCommand('JSON.GET', [`chat:${key}`])) {
...
async function exitSocket(socket: Socket, key: string) {
  ...
  if (!await redis.sendCommand('JSON.GET', [`socket:${socket.id}`])) {
  ...
  let reply = await redis.sendCommand('JSON.GET', [`socket:${socket.id}`, '.name', '.uid', '.key']);
  ...
  const { name, uid } = reply as unknown as { name: string, uid: string, key: string };
  ...
  const activeUsers = reply as unknown as number;
  ...
  if (activeUsers <= 0) {
    ...
    const users = reply as unknown as { [key: string]: User };
